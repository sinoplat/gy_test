...
import com.sinosoft.mobileshop.service.VpnWorkService.TaskSubmitter;
import com.sinosoft.mobileshop.service.VpnWorkService.TaskTracker;
...
//	private static final String LOGTAG = "syso";
	...
	public XmppManager(VpnWorkService vpnWorkService) {
		context = vpnWorkService;
		// 获取Task提交管理器，用于维护并行任务
		taskSubmitter = vpnWorkService.getTaskSubmitter();
		// Task计数器
		taskTracker = vpnWorkService.getTaskTracker();
		// 配置信息
		sharedPrefs = vpnWorkService.getSharedPreferences();
		xmppHost = sharedPrefs.getString(Constants.XMPP_HOST,xmppHost1);
		xmppPort = sharedPrefs.getInt(Constants.XMPP_PORT,xmppPort1);
		Log.d(LOGTAG, "xmppHost:113:"+xmppHost);
		Log.d(LOGTAG, "xmppHost:114:"+xmppPort);
	...
	private void submitConnectTask() {
		Log.d(LOGTAG, "submitConnectTask()44444...");
		// 连接
	...
	private void submitRegisterTask() {
		Log.d(LOGTAG, "submitRegisterTask()33333...");
		// 连接
		submitConnectTask();
	...
	private void submitLoginTask() {
		Log.d(LOGTAG, "submitLoginTask()2222...");
		// 注册
		submitRegisterTask();
		// 登录
	...
	private class ConnectTask implements Runnable {
		...
		public void run() {
			Log.i(LOGTAG, "ConnectTask.run()55555...");
	...
	private class RegisterTask implements Runnable {
		...
		public void run() {
			Log.i(LOGTAG, "RegisterTask.run()666666...");
			...
			if (!xmppManager.isRegistered()) {
				...
				Log.i(LOGTAG, "newUsername:434:" + newUsername);
				// final String newUsername = newRandomUUID();
				// 密码
				final String newPassword = newRandomUUID();
				Log.i(LOGTAG, "newPassword:437:" + newPassword);
						...
						if (packet instanceof IQ) {
							...
							if (response.getType() == IQ.Type.ERROR) {
								if (!response.getError().toString().contains("409")) {
									Log.e("syso","Unknown error while registering XMPP account! "
													+ response.getError()
															.getCondition());
								}
								...
								Log.d("syso", "username=466" + newUsername);
								Log.d("syso", "password=467" + newPassword);
								...
								Log.i("syso", "Account registered successfully");
								xmppManager.runTask();
							}
						}
					}
				...
				String xml = registration.toXML();
				Log.i(LOGTAG, "注册的xml："+xml);
	...
	private class LoginTask implements Runnable {
		...
		public void run() {
			...
			if (!xmppManager.isAuthenticated()) {
				Log.d(LOGTAG, "username=520:" + username);
				Log.d(LOGTAG, "password=521:" + password);

				try {
					xmppManager.getConnection().login(
							xmppManager.getUsername(),
							xmppManager.getPassword(), XMPP_RESOURCE_NAME);
					Log.d(LOGTAG, "Loggedn in successfully*******527");
