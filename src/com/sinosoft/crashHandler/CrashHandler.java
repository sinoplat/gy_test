...
//		}.start();
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		try {
			...
			GetUserCode users  = new GetUserCode(mContext);
			userCode = users.getUsercode();
			String time = formatter.format(new Date());
			fileName = "crash-" + time + "-" + userCode + ".txt";
			SharedPreferences   user = mContext.getSharedPreferences("SP", Context.MODE_PRIVATE);
			...
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				...
				Log.i("sysy", "崩溃日志储存路径："+path + fileName);
