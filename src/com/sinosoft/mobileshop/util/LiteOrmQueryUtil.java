...
public class LiteOrmQueryUtil {
	private LiteOrm liteOrm;

	public LiteOrmQueryUtil(LiteOrm liteOrm) {
	 ...
	 * @return
	 */
	public <T> List<T> getQueryAllOrderDesc(Class<T> cla,String column) {
		QueryBuilder queryBuilder = new QueryBuilder<T>(cla).appendOrderDescBy(column);
	...
	public <T> List<T> getQueryByWhere(Class<T> cla, String field,
			String[] value) {
		return liteOrm.<T> query(new QueryBuilder(cla).where(field + "=?",
				value));
	}
