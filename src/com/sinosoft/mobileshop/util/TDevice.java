 ...
 * 手机设备工具类 =========================================
 * 
 * Copyright： micat Author： Jerry.Xu Version： 1.0 CreateTime：上午10:05:19 Desc:
 * Revision History:
 *
 * =========================================
 */
...
public class TDevice {
	...
	public static int getDefaultLoadFactor() {
		if (_loadFactor == null) {
			Integer integer = Integer.valueOf(0xf & App.context()
	...
	public static DisplayMetrics getDisplayMetrics() {
		...
		((WindowManager) App.context().getSystemService(Context.WINDOW_SERVICE))
	...
	public static int getStatusBarHeight() {
		...
		try {
			...
			return App.context().getResources().getDimensionPixelSize(x);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0;
	...
	public static String getUdid() {
		String udid = App.getPreferences().getString("udid", "");
		if (udid.length() == 0) {
	...
	public static boolean hasBigScreen() {
		...
		if (_hasBigScreen == null) {
			boolean flag1;
			if ((0xf & App.context().getResources().getConfiguration().screenLayout) >= 3)
				flag1 = flag;
			else
				flag1 = false;
	...
	public static final boolean hasCamera() {
		if (_hasCamera == null) {
			PackageManager pckMgr = App.context().getPackageManager();
			boolean flag = pckMgr
	...
	public static boolean isPackageExist(String pckName) {
		try {
			PackageInfo pckInfo = App.context().getPackageManager()
	...
	public static void hideSoftKeyboard(View view) {
		if (view == null)
			return;
		((InputMethodManager) App.context().getSystemService(
	...
	public static boolean isPortrait() {
		boolean flag = true;
		if (App.context().getResources().getConfiguration().orientation != 1)
			flag = false;
		return flag;
	}
	...
	public static String getCurCountryLan() {
		return App.context().getResources().getConfiguration().locale
				.getLanguage()
				+ "-"
				+ App.context().getResources().getConfiguration().locale
						.getCountry();
	}

	public static boolean isZhCN() {
		String lang = App.context().getResources().getConfiguration().locale
				.getCountry();
	...
	public static PackageInfo getPackageInfo(String pckName) {
		try {
			return App.context().getPackageManager().getPackageInfo(pckName, 0);
		} catch (NameNotFoundException e) {
			TLog.error(e.getMessage());
		}
		return null;
	...
	public static int getVersionCode() {
		...
		try {
			versionCode = App.context().getPackageManager()
					.getPackageInfo(App.context().getPackageName(), 0).versionCode;
	...
	public static String getVersionName() {
		String name = "";
		try {
			name = App.context().getPackageManager()
	...
	public static boolean isScreenOn() {
		PowerManager pm = (PowerManager) App.context().getSystemService(
	...
	public static String getIMEI() {
		TelephonyManager tel = (TelephonyManager) App.context()
				.getSystemService(Context.TELEPHONY_SERVICE);
		return tel.getDeviceId().trim();
	}

	public static String getPhoneType() {
		String ss = android.os.Build.MODEL;
		String replace = ss.replace(" ", "");
		return replace.trim();
	}

	public static void openApp(Context context, String packageName) {
		Intent mainIntent = App.context().getPackageManager()
	...
	public static void openApp(Context context, String packagename,String activityName) {
		try {
			...
			e.printStackTrace();
			StytledDialog.showMdAlert(context, "提醒", "未找到应用启动页面，请确认", "确定", true, true, null);
		}
	}
	...
	public static boolean is3GOpen() {
	...
	public static int getStatuBarHeight() {
		...
		try {
			...
			field = c.getField("status_bar_height");
			x = Integer.parseInt(field.get(obj).toString());
			sbar = App.context().getResources().getDimensionPixelSize(x);
